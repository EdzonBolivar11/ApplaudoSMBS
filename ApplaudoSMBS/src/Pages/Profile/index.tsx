...
const index: FunctionComponent<ProfileProps> = (props) => {
  ...
  const colors = [Colors.gradient1, Colors.gradient2, Colors.gradient3, Colors.gradient4, Colors.gradient5];
  ...
  return (
    <Screen>
      ...
      <Button iconLeft onPress={() => Linking.openURL(`whatsapp://send?text='Prueba técnica'&phone=${PersonalInformation.phone}`)} style={styles.button}>
          <Icon name='logo-whatsapp' style={{ color: Colors.white }} />
          <Text style={{color: Colors.white}}>Contactar</Text>
      </Button>
      <View style={styles.infoWrapper}>
        {
          PersonalInformation.data.map(section => <Section key={section.id} section={section} />)
        }
      </View>
      <Button style={styles.nextButton} onPress={onPressNext}>
